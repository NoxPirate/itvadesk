...
def update_ticket(ticket_id):
    ...
    try:
        if t.status == "Resolved":
            if t.email:
                print(f"DEBUG: Attempting to send resolution email to {t.email}...")
                subject = "Your Ticket Has Been Resolved"
                body = f"Hello {t.user_name},\n\nYour ticket (ID: {t.id}) has been marked as Resolved.\n\nIssue:\n{t.issue}\n\nThanks,\nIT Support Team"
                msg = Message(subject=subject, recipients=[t.email], body=body)
                mail.send(msg)
                print("DEBUG: Resolution email sent successfully!")
            else:
                print("DEBUG: Ticket has no email address, skipping notification.")
    except Exception as e:
        print(f"DEBUG: Error sending resolution email: {e}")
        pass
